...
export default function Plan() {
    ...
    return (
        <div id="plan" className="bg-[url('/images/background-plan.png')] bg-cover py-10 md:py-20 px-5">
            <div className="w-full max-w-[1106px] mx-auto mb-8">
                <SectionTitle>武将プラン</SectionTitle>
                <div ref={animateRefs} className="rounded-[20px] bg-black/50 px-4 md:px-7 mt-8 md:mt-16 pt-6 md:pt-12">
                    <div className="flex md:items-start max-md:flex-col items-center md:gap-[60px]">
                        <div ref={animateRefs} className="flex flex-col justify-center items-center">
                            <p className="text-white md:text-[70px] text-[40px] font-bold mb-4">27,500 <span className="md:text-[52px] text-[28px]">円</span></p>
                            <Link href="/" className="md:w-[300px] w-[240px] h-[60px] md:h-[70px] flex justify-center items-center text-white bg-[#D80000] md:text-[20px] font-bold text-[14px] rounded-[50px] relative [box-shadow:0px_5.579px_0px_0px_#850403] duration-150 hover:opacity-75">
                                <span>ご購入はこちら</span>
                        ...
                        <div ref={animateRefs} className="pt-4 max-md:flex max-md:flex-col max-md:items-center">
                    ...
                    <p ref={animateRefs} className="md:text-[14px] text-[12px] text-white text-right px-3 pb-5 max-md:pt-5">※画像はイメージです、実際に届くNFTと違う場合があります。</p>
                </div>
            </div>
            <div className="flex justify-center md:mb-[-135px] mb-[-70px] relative">
                <img ref={animateRefs} className="max-md:w-[43px]" src="/images/arrow-down.png" alt="" />
            </div>
        </div>
    )
  }
